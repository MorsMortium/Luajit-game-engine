-- gsl.lua
--
-- Cdefs and load of the GSL (GNU Scientific Library).
-- Directly access functions in the GSL C Library.
--
...
if jit.os == "Linux" or jit.os == "BSD" then
  ...
  if err~=nil and ffi.string(err):find("invalid flags to dlopen") then
    ffi.C.dlopen(libblas, 2) -- RTLD flags are enums on Android
    err = ffi.C.dlerror()
  end
  if err~=nil then
