return function(args)
  ...
  function GiveBack.Start(Configurations)
    ...
    InputParameters[0].channelCount =
    PortAudio.dll.Pa_GetDeviceInfo(InputParameters[0].device).maxInputChannels
    InputParameters[0].sampleFormat = PortAudio.dll.paFloat32
    InputParameters[0].suggestedLatency =
    PortAudio.dll.Pa_GetDeviceInfo(InputParameters[0].device).defaultHighInputLatency
    OutputParameters[0].device = PortAudio.dll.Pa_GetDefaultHostApi()
    OutputParameters[0].channelCount =
    PortAudio.dll.Pa_GetDeviceInfo(InputParameters[0].device).maxOutputChannels
    OutputParameters[0].sampleFormat = PortAudio.dll.paFloat32
    OutputParameters[0].suggestedLatency =
    PortAudio.dll.Pa_GetDeviceInfo(OutputParameters[0].device).defaultHighOutputLatency
