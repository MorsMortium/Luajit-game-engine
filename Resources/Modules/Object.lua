...
function GiveBack.Create(GotObject, Parent, Arguments, HelperMatrices)
	...
	Arguments[1], Arguments[2], Arguments[3], Arguments[5], Arguments[6], Arguments[7]
	...
	-- The Direction of the shape, it will come from the Translation,
	-- and will go to the Angle of the last three lines, Default: Up
	...
	-- Default shape: All Point is perpendicular to the first
	...
	if type(GotObject) == "table" then
		...
		if IsVector3(GotObject.JointSpeed) then
			Object.JointSpeed = GotObject.JointSpeed
		end
		if IsVector3(GotObject.JointRotationSpeed) then
	    Object.JointRotationSpeed =
			EulerToQuaternion(GotObject.JointRotationSpeed)
	  end
		...
		if GoodTypesOfTable(Object.VisualLayers, "string") then
			Object.VisualLayers = GotObject.VisualLayers
		end
		if GoodTypesOfTable(Object.PhysicsLayers, "string") then
		...
		if type(GotObject.CollisionDecay) == "number" and
		0 <= GotObject.CollisionDecay and GotObject.CollisionDecay <= 1 then
			Object.CollisionDecay = GotObject.CollisionDecay
		end
		if GoodTypesOfTable(GotObject.CollisionReaction, "number") and
		#GotObject.CollisionReaction == 2 then
	  ...
	  if type(GotObject.Powers) == "table" then
		...
		ORender.DataCheck(Object.RenderData, GotObject.RenderData, ObjectRenderGive)
	else
		local ORender = ObjectRender.Library.ObjectRenders[Object.ObjectRenderer]
		ORender.DataCheck(Object.RenderData, nil, ObjectRenderGive)
	end
	for ak = 0, 2 do
	...
	General.Library.UpdateObject(Object, true, HelperMatrices, GeneralGive)
	return Object
end
function GiveBack.Copy(GotObject, Parent, Arguments, HelperMatrices)
	...
	Object.JointSpeed = {GotObject.JointSpeed[1],
											GotObject.JointSpeed[2],
											GotObject.JointSpeed[3]}
	...
	Object.ObjectRenderer = GotObject.ObjectRenderer
	Object.CollisionDecay = GotObject.CollisionDecay
	Object.CollisionReaction = {GotObject.CollisionReaction[1],
															GotObject.CollisionReaction[2]}
