...
local function normalise(a, General)
...
local function update_simplex3(a, b, c, d, simp_dim, search_dir, General)
  ...
  --  | /
  --  c
  local n = CrossProduct(VectorSubtraction(b, a), VectorSubtraction(c, a)); --triangle's normal
  local AO = MinusVector(a); --direction to origin
  --Determine which feature is closest to origin, make that the new simplex
  simp_dim[1] = 2;
  if(General.Library.DotProduct(CrossProduct(VectorSubtraction(b, a), n), AO)>0)then --Closest to edge AB
    VectorCopy(c, a);
    --simp_dim[1] = 2;
    VectorCopy(search_dir, CrossProduct(CrossProduct(VectorSubtraction(b, a), AO), VectorSubtraction(b, a)));
    return;
  end
  if(General.Library.DotProduct(CrossProduct(n, VectorSubtraction(c, a)), AO)>0)then --Closest to edge AC
    VectorCopy(b, a);
    --simp_dim[1] = 2;
...
function GiveBack.EPA(a, b, c, d, coll1, coll2, support1, support2, General)
  ...
  faces[1][2] = b;
  faces[1][3] = c;
  faces[1][4] = normalise(CrossProduct(VectorSubtraction(b, a), VectorSubtraction(c, a)), General); --ABC
  faces[2][1] = a;
  faces[2][2] = c;
  faces[2][3] = d;
  faces[2][4] = normalise(CrossProduct(VectorSubtraction(c, a), VectorSubtraction(d, a)), General); --ACD
  faces[3][1] = a;
  faces[3][2] = d;
  faces[3][3] = b;
  faces[3][4] = normalise(CrossProduct(VectorSubtraction(d, a), VectorSubtraction(b, a)), General); --ADB
  faces[4][1] = b;
  faces[4][2] = d;
  faces[4][3] = c;
  faces[4][4] = normalise(CrossProduct(VectorSubtraction(d, b), VectorSubtraction(c, b)), General); --BDC
  local num_faces=4;
  local closest_face;
  for iterations=1, EPA_MAX_NUM_ITERATIONS do
      ...
      if(General.Library.DotProduct(faces[i][4], VectorSubtraction(p, faces[i][1] ))>0) then--triangle i faces p, remove it
        ...
        for j=1, 3 do--Three edges per face
          ...
          while k <= num_loose_edges do--Check if current edge is already in list
            if(vectorequals(loose_edges[k][2],current_edge[1]) and vectorequals(loose_edges[k][1], current_edge[2]))then
    ...
    for i=1, num_loose_edges do
      ...
      faces[num_faces][3] = p;
      faces[num_faces][4] = normalise(CrossProduct(VectorSubtraction(loose_edges[i][1], loose_edges[i][2]), VectorSubtraction(loose_edges[i][1], p)), General);
...
function GiveBack.GJK(coll1, coll2, support1, support2, General, mtv)--(Collider* coll1, Collider* coll2, vec3* mtv=NULL);
  ...
  if(search_dir=={0,0,0})then --origin is on this line segment
    ...
    search_dir = CrossProduct(c-b, {1,0,0}); --normal with x-axis
    if(search_dir=={0,0,0}) then search_dir = CrossProduct(c-b, {0,0,-1});end --normal with z-axis
  end
  ...
  for iterations=1, GJK_MAX_NUM_ITERATIONS do
    ...
    elseif(update_simplex4(a,b,c,d,simp_dim,search_dir, General)) then
      if mtv then mtv[1] = GiveBack.EPA(a,b,c,d,coll1,coll2, support1, support2, General) end
      return true;
    end
  end--endfor
  return false;
